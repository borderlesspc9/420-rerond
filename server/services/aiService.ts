...
async function pdfParaImagens(filePath: string): Promise<string[]> {
  ...
  const outputPrefix = path.join(tempDir, path.basename(filePath, path.extname(filePath)))
  ...
  try {
    ...
    console.log(`📄 PDF: ${path.basename(filePath)} | Páginas convertidas: ${totalPaginas}`)
...
export async function analisarSolicitacaoComIA(
  ...
  const pdfPaths = params.arquivosPaths.filter(p => p.toLowerCase().endsWith('.pdf'))

  console.log(`\n🤖 Iniciando análise com IA`)
  console.log(`   Provider: ${config.provider} | Modelo: ${config.model}`)
  console.log(`   Limite de tokens (MAX_TOKENS_ANALISE): ${MAX_TOKENS_ANALISE}`)
  ...
  if (pdfPaths.length > 0 && config.provider === 'groq') {
    console.warn(`⚠️  Groq não suporta visão. PDFs serão ignorados. Configure OPENAI_API_KEY para análise visual dos documentos.`)
  }
...
async function analisarComVision(
  ...
  for (const pdfPath of pdfPaths) {
    ...
    try {
      ...
      for (let i = 0; i < imagens.length; i++) {
        ...
        content.push({
          type: 'image_url',
          image_url: { url: `data:image/jpeg;base64,${imagens[i]}`, detail: 'auto' },
        })
      }
    } catch (error: any) {
      console.error(`❌ Erro ao processar PDF ${path.basename(pdfPath)}: ${error.message}`)
      content.push({ type: 'text', text: `[Erro ao processar ${path.basename(pdfPath)}: ${error.message}]` })
    }
  }
  ...
  if (MOCK_OPENAI_VISION) {
    console.warn('🧪 MOCK_OPENAI_VISION=true: simulando resposta do GPT-4o Vision (sem chamada externa).')
    const mockResponse = {
      LOCALIZACAO: 'ok',
      KM_INICIO: 'ok',
      KM_FIM: 'ok',
      ...
      _observacao: 'Resposta simulada para teste de fluxo sem consumir cota da OpenAI.',
    }
  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'Você é um especialista em análise de projetos de infraestrutura rodoviária. Analise visualmente os documentos e forneça análises detalhadas e precisas.',
        },
        { role: 'user', content },
      ],
    ...
    console.log(`   Tokens usados — prompt: ${tokensPrompt} | resposta: ${tokensResposta} | total: ${tokensUsados}`)
    console.log(`   Custo estimado: ~$${((tokensPrompt * 0.000005) + (tokensResposta * 0.000015)).toFixed(4)} USD`)
    console.log(`\n📋 Resposta da IA:\n${resultado}\n`)
    ...
    if (error.code === 'insufficient_quota') throw new Error('Cota da API OpenAI esgotada. Verifique sua conta.')
    if (error.status === 429) throw new Error('Limite de requisições do OpenAI excedido. Tente novamente mais tarde.')
    if (error.code === 'invalid_api_key') throw new Error('Chave da API OpenAI inválida.')
...
async function analisarSemVision(
  ...
  const messages: ChatMessage[] = [
    {
      role: 'system',
      content: 'Você é um especialista em análise de projetos de infraestrutura rodoviária.',
    },
  ...
  try {
    const completion =
      config.provider === 'groq'
        ? await groqChatCompletion(messages, config.model, { temperature: 0.2, maxTokens: MAX_TOKENS_ANALISE })
        : await openaiChatCompletion(messages, config.model, { temperature: 0.2, maxTokens: MAX_TOKENS_ANALISE })
